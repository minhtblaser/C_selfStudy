...
{ //loại bỏ node lá bên trái cho node
    if (root != NULL)
    { //nếu cây không rỗng
        if (root->data == value) //th1
        { //nếu tìm thấy node có giá trị value
            if (root->left == NULL) //th2
            { //nếu node không có node con trái
                cout << "Node " << value << " không có lá trái" << endl;
                return;
            }
            else if ((root->left)->left != NULL || (root->left)->right != NULL) //th3 
            {
            ...
            else if ((root->left)->left == NULL && (root->left)->right == NULL)
            {
                ...
                delete (root->left); //           //giải phóng node
                return;
            }
        }
...
{ //loại bỏ cả cây gốc root
    if (root != NULL) //dừng khi danh sách rỗng.
    {                           
